...
namespace yugop.connection
{
    ...
    public struct MidiNote
    {
        ...
        public string TrackName; // Ableton トラック名（OSC 経路のみ。空文字 = 不明）
        public int TrackIndex;   // Ableton トラック index（OSC 経路のみ。-1 = 不明）
        public int SlotIndex;    // 親クリップ slot index（OSC 経路のみ。-1 = 不明）
        public string ClipName;  // 親クリップ名（OSC 経路のみ。空文字 = 不明）
