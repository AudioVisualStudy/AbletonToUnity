...
「MIDI を受信してオブジェクトを動かす」「音声を波形として表示する」「OSC で Live のセッション情報とリアルタイム MIDI を受け取る」といった基礎を、初心者の学生でも追いやすい形でまとめています。
...
- **3 OSC 入力を視覚化する**：Ableton Live から **AbletonOSC**（セッション情報）と **Y-OSC-NoteSender**（リアルタイム MIDI）を UDP で受け取り、**OscHub** が 1 つの入口にまとめる。Console ログや `AddNoteOnListener` でイベントを確認できる。

いずれも「**データの取得元を 1 つにまとめ、複数のビジュアライザーで使い回す**」構成にしています。
...
│
├── 03. OSC/                     # 3 OSC 入力を視覚化する
│   ├── OscHubの使い方.md        # OscHub API 解説・Live 側セットアップ
│   └── Lib/
│       ├── OscHub/              # OSC 受信の中核（AbletonOSC + Y-OSC-NoteSender）
│       └── Y-OscNoteSender/     # Live 側 Max MIDI Effect（Distribution）
│
...
### 3. OSC 入力を視覚化する：「UDP 受信 → セッション状態 + リスナーに配信」

1. **AbletonOSC**  
   Live の Control Surface として設定し、セッション情報（トラック・クリップ・MIDI ノート・再生位置など）を UDP で送受信します（既定 11000 / 11001）。

2. **Y-OSC-NoteSender**  
   Live の各 MIDI トラックに配置する Max MIDI Effect です。リアルタイムの Note On を `/y-osc/note` として Unity へ送ります。

3. **OscHub**  
   AbletonOSC と Y-OSC-NoteSender の両方を受け取り、**`LiveSessionState`** にセッション全体を保持します。リアルタイム MIDI は **`AddNoteOnListener`** で MidiHub と同様にリスナー登録できます。  
   - シーンに **1 つ** OscHub を置き、**`OscHub.Instance`** で参照します。  
   - `MidiNote` 型は `01. MIDI` の MidiHub と共有しています（OSC 経路では `TrackIndex` / `ClipName` なども付与されます）。

**まとめ**  
「**AbletonOSC + Y-OSC-NoteSender → OscHub（受信・配信）→ 各スクリプトの登録した関数 / Session 参照**」です。仮想 MIDI ポートは不要で、Live と Unity は UDP で直接つながります。
...
1. Live 側で **AbletonOSC**（Control Surface）と **Y-OSC-NoteSender**（M4L デバイス）を設定する。手順は **`Assets/03. OSC/OscHubの使い方.md`** を参照。  
2. 任意のシーンに空の GameObject を作成し、**OscHub** コンポーネントをアタッチする。  
3. **Play** して Live でクリップを再生・MIDI を入力する。  
4. Console に `[OscHub] scan done` や `[OscHub] live note` が出れば連携成功。
...
- **OSC**：AbletonOSC（Live Control Surface）と Y-OSC-NoteSender（Max MIDI Effect）。UDP ポート 11000 / 11001 が空いていること
...
| **OscHub** | OSC（UDP）を受信し、セッション状態とリアルタイム MIDI を配信するコンポーネント。シングルトン。 |
| **AbletonOSC** | Live のセッション情報を OSC で送受信する Control Surface（外部ツール）。 |
| **Y-OSC-NoteSender** | Live の MIDI トラックに置く Max MIDI Effect。リアルタイム Note On を Unity へ送る。 |
...
- **3 OSC 入力を視覚化する**：AbletonOSC + Y-OSC-NoteSender → **OscHub** → `Session` 参照 / `AddNoteOnListener` でイベントが届く。  
- いずれも「**データの入り口を 1 つにまとめ、複数のビジュアライザーで共有する**」構成になっており、初心者の方はまずこの流れを押さえると読みやすくなります。  
